...
export default {
  ...
  props: {
    name: String,
    // Prop to pass the initial data for each course (from parent)
    initialData: {
      type: Object,
      required: true,
    },
  ...
  data() {
    return {
      courseData: { ...this.initialData } // Make a local copy of the passed data
    };
  },
  methods: {
    // Emit the updated course data to the parent
    submitCourse() {
      this.$emit("update-course", { index: this.index, data: this.courseData });
    }
  },
  setup() {
  ...
  <div>
    ...
    <RadioGroup v-model="mem" class="mt-2" name="rating">
      ...
      <div class="grid grid-cols-5 gap-3 sm:grid-cols-5">
        <RadioGroupOption as="template" v-for="option in ratingOptions" v-model="courseData.rating" :key="option.name" :value="option.value"
          v-slot="{ checked }">
          <div
        ...
        <Disclosure v-slot="{ open }">
          ...
          <DisclosurePanel class="px-4 pb-2 pt-4 text-sm text-gray-500">
            <div>
              ...
              <div class="mt-1">
                <textarea rows="4" name="comment" id="comment" v-model="courseData.comment"
