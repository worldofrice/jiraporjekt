...
<template>
  <div>
    ...
    <p class="mt-2 text-sm text-red-600" v-if="props.errors">
      {{ props.errors[0] }}
    </p>
    ...
    <RadioGroup v-model="selectedRating" class="mt-2" name="rating">
      ...
      <div class="grid grid-cols-5 gap-3 sm:grid-cols-5">
        <RadioGroupOption as="template" v-for="option in ratingOptions" :key="option.name" :value="option.value">
          <div
            :class="['cursor-pointer focus:outline-none', selectedRating === option.value ? 'bg-indigo-600 border-transparent text-white hover:bg-indigo-700' : 'bg-white border-gray-300 text-gray-900 hover:bg-gray-50', 'border rounded-md py-3 px-3 flex items-center justify-center text-sm font-medium uppercase sm:flex-1']">
            ...
            <div>
              ...
              <div class="mt-1">
                <textarea v-model="comment" rows="4" name="comment" id="comment"
                  class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-300 rounded-md text-gray-900" />
              </div>
            </div>
          </DisclosurePanel>
        </Disclosure>
